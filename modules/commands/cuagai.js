var request = require("request");const { readdirSync, readFileSync, writeFileSync, existsSync, copySync, createWriteStream, createReadStream } = require("fs-extra");
    module.exports.config = {
        ...
        hasPermssion: 0,
        credits: "Anonymous",
        description: "Game bầu cua vip ",
        commandCategory: "Nsfw",
        usages: "<[đỏ/cam/tím/xanh/đen/trắng] hoặc[❤️/🧡/💜/💙/🖤/🤍]> <Số tiền cược (lưu ý phải trên 5000$)>",
        cooldowns: 5
    };
    ...
    module.exports.onLoad = async function () {
        ...
        if (!existsSync(__dirname + '/cache/trang.jpg')) {
            request('https://i.imgur.com/380RNm0.jp').pipe(createWriteStream(__dirname + '/cache/trang.jpg'));
        }
...
var full = [];
    module.exports.run = async function({ api, event, args, Currencies }) { var out = (msg) => api.sendMessage(msg,event.threadID, event.messageID);
        const slotItems = ["do", "cam", "tim", "xanh", "den", "trang"];
            ...
            if (moneyBet < 5000) return api.sendMessage("[𝑩𝑶𝑻] => Số tiền đặt không được dưới 5000 đô!", event.threadID, event.messageID);
    ...
    for (let i = 0; i < 3; i++) number[i] = slotItems[Math.floor(Math.random() * slotItems.length)];
        var itemm;
            ...
            api.sendMessage({body:"🌺 𝑷𝒉𝒆́𝒑 𝑻𝒉𝒖𝒂̣̂𝒕 𝑾𝒊𝒏𝒙 𝑬𝒏𝑪𝒉𝒂𝒏𝑻𝒊𝒙...𝑩𝒊𝒆̂́𝒏 𝑯𝒊̀𝒏𝒉\n🌺 𝑪𝒉𝒖́𝒄 𝑩𝒂̣𝒏 𝑴𝒂𝒏𝒈 𝑽𝒆̂̀ 𝑴𝒐̣̂𝒕 𝑪𝒐̂ 𝑽𝒐̛̣ 𝑿𝒊𝒏𝒉 𝑽𝒂̀ 𝑫𝒂𝒎𝒅𝒂𝒏𝒈 😍",attachment: createReadStream(__dirname + "/cache/cuagai.gif")},event.threadID,async (error,info) => {
            ...
            if (i == 1) {
                ...
                var mon = parseInt(args[1]) * 4; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
                        return api.sendMessage({body:`➢ 𝑩𝒊𝒆̂́𝒏 𝑯𝒊̀𝒏𝒉: ${full.join(" | ")}\n🌺 𝑩𝒂̣𝒏 𝑽𝒖̛̀𝒂 𝑽𝒖̛̀𝒂 𝑪𝒖𝒂 𝑫𝒊́𝒏𝒉 𝑴𝒐̣̂𝒕 𝑬𝒎 𝑽𝒂̀ 𝑴𝒂𝒏𝒈 𝑴𝒐̣̂𝒕 𝑵𝒂̀𝒏𝒈 𝑫𝒂̂𝒖 𝑽𝒆̂̀ 𝑪𝒉𝒐 𝑴𝒆̣ 😍\n𝑪𝒐̣̂𝒏𝒈 ${mon}$ 𝑽??̀𝒐 𝑻𝒂̀𝒊 𝑲𝒉𝒐𝒂̉𝒏 𝑪𝒖̉𝒂 𝑩𝒂̣𝒏 💸\n➢ 𝑪𝒖𝒂 𝑫𝒊́𝒏𝒉 𝑬𝒎 𝑮𝒂́𝒊 𝑴𝒂̀𝒖 ${icon}`,attachment: listimg},event.threadID, event.messageID);
            }
            else return api.sendMessage("[ 𝑩𝑶𝑻 🐧] => 𝑳𝒐̂̃𝒊 𝒓𝒐̂̀𝒊 𝒄𝒖𝒂 𝒈𝒂́𝒊 𝒃𝒐̛́𝒕 𝒏𝒈𝒉𝒊𝒆̣̂𝒏 𝒍𝒂̣𝒊",event.threadID,event.messageID);
        } else  {
            await Currencies.decreaseMoney(event.senderID, parseInt(args[1])); console.log('s4')
            return api.sendMessage({body:`➢ 𝑩𝒊𝒆̂́𝒏 𝑯𝒊̀𝒏𝒉: ${full.join(" | ")}\n🌺 𝑩𝒂̣𝒏 𝑸𝒖𝒂́ 𝑿𝒂̂́𝒖 𝑻𝒓𝒂𝒊 𝑽𝒂̀ 𝑩𝒊̣ 𝑬𝒎 𝑮𝒂́𝒊 𝑨̂́𝒚 𝑺𝒖́𝒕 😭\n𝑻𝒓𝒖̛̀ ${args[1]}$ 𝑽𝒂̀𝒐 𝑻𝒂̀𝒊 𝑲𝒉𝒐𝒂̉𝒏 𝑪𝒖̉𝒂 𝑩𝒂̣𝒏 💸\n➢ 𝑽𝒊̀ 𝑩𝒂̣𝒏 𝑲𝒉𝒐̂𝒏𝒈 𝑻𝒉𝒆̂̉ 𝑻𝒂́𝒏 𝑫𝒊́𝒏𝒉 𝑬𝒎 𝑴𝒂̀𝒖 ${icon}`,attachment: listimg},event.threadID, event.messageID);
        }
            } ,event.messageID);
    };
