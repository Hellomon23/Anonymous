...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {  
    ...
    switch (args[0]) {
        ...
        case "add": { 
            if (event.senderID != 100073383848952) return api.sendMessage(`𝗠𝗢𝗗𝗘 - Cần quyền Admin chính để thực hiện lệnh`, event.threadID, event.messageID)
        ...
        case "rm":
        case "delete": {
            if (event.senderID != 100073383848952) return api.sendMessage(`𝗠𝗢𝗗𝗘 - Cần quyền Admin để thực hiện`, event.threadID, event.messageID)
   ...
   if (permssion < 2) return api.sendMessage("𝗠𝗢𝗗𝗘 - Cần quyền Người hỗ trợ trở lên để thực hiện", threadID, messageID);       
        ...
        case '-o': {
            //---> CODE ADMIN ONLY<---//
          if (permssion != 2) return api.sendMessage("𝗠𝗢𝗗𝗘 - Cần quyền Admin để thực hiện", threadID, messageID);
